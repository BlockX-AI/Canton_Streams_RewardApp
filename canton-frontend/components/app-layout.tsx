...
import {
  ...
  Medal, Zap, Sprout, ArrowLeftRight,
} from 'lucide-react';
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const { partyId, disconnect, status } = useWallet();
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="h-14 border-b border-provn-border flex items-center px-4 lg:px-6 bg-provn-surface/80 backdrop-blur-sm flex-shrink-0 relative z-10">
          ...
          <div className="flex items-center gap-3">
            {partyId ? (
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-mono bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                <Wallet className="w-3.5 h-3.5" />
                <span className="hidden sm:inline">{shortenAddress(partyId)}</span>
              </div>
            ) : (
              <button
                onClick={() => setWalletOpen(true)}
                className="flex items-center gap-2 px-4 py-1.5 rounded-lg text-xs font-medium bg-gradient-to-r from-emerald-500 to-emerald-600 text-white hover:from-emerald-600 hover:to-emerald-700 transition-all"
              >
                <Wallet className="w-3.5 h-3.5" />
                Connect
              </button>
            ...
            <div className="flex items-center gap-2 text-xs text-provn-muted">
              <span className="hidden sm:inline">Canton DevNet</span>
