...
const CANTON_API = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'
...
const demoScenarios: Record<string, { sender: string; receiver: string; rate: string; ratePerSec: number; buffer: string; label: string; statusColor: string }> = {
  Payroll: { sender: "company::party", receiver: "alice::party", rate: "$0.00027/sec", ratePerSec: 0.00027, buffer: "8h remaining", label: "Streaming", statusColor: "#10b981" },
  Subscriptions: { sender: "user::party", receiver: "service::party", rate: "$0.00008/sec", ratePerSec: 0.00008, buffer: "24h remaining", label: "Active", statusColor: "#8b5cf6" },
  Bounties: { sender: "dao::party", receiver: "dev::party", rate: "$0.00019/sec", ratePerSec: 0.00019, buffer: "4h remaining", label: "Streaming", statusColor: "#10b981" },
  Splits: { sender: "protocol::party", receiver: "team::party", rate: "$0.00031/sec", ratePerSec: 0.00031, buffer: "12h remaining", label: "Routing", statusColor: "#f59e0b" },
}

const protocolStatus = [
  { icon: Radio, label: "Status", value: "Canton DevNet (Live)", color: "text-emerald-400", borderColor: "border-emerald-500/20", bgColor: "bg-emerald-500/5" },
...
const protocolFeatures = [
  ...
  { icon: Zap, title: "Per-Second Settlement", description: "Powered by Canton's sub-second DAML execution. Real-time payments at scale." },
]

const steps = [
  { step: "00", title: "Install SDK", description: "Install the GrowStreams SDK and initialize the client for Canton DevNet.", code: `npm i @growstreams/sdk\n\nimport { GrowStreams } from '@growstreams/sdk';\nconst client = new GrowStreams({\n  network: "canton-devnet"\n});` },
...
export default function HomeV2() {
  ...
  useEffect(() => {
    ...
    async function loadActive() {
      try {
        const res = await fetch(`${CANTON_API}/campaigns`, { cache: 'no-store' }).then(r => r.json()).catch(() => ({ campaigns: [] }))
            ...
            <div className="space-y-6">
              ...
              >
                <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-emerald-500/20 text-emerald-400 text-xs font-medium tracking-wider uppercase">
                  ...
                  <TextScramble text="Live on Canton DevNet" speed={25} scrambleDuration={1200} />
                </div>
              </motion.div>
              ...
              >
                Real-time token streaming on Canton Network. DAML smart contracts
        ...
        <div className="max-w-6xl mx-auto">
          ...
          >
            ...
            <p className="text-lg text-gray-400 max-w-2xl mx-auto leading-relaxed">
              A generalized streaming smart-contract system on Canton.
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 border-y border-provn-border/20 relative overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto relative z-10">
          ...
          >
            ...
            <p className="text-base text-gray-400 leading-relaxed">Four steps to start streaming payments on Canton.</p>
          </motion.div>
              ...
              <div className="sticky top-28 space-y-4">
                <div className="p-4 rounded-xl bg-provn-surface/40 border border-provn-border/30">
                  ...
                  <div className="mt-3 pt-3 border-t border-provn-border/30">
                    ...
                    <div className="text-xs text-gray-300 font-medium mt-0.5">Canton DevNet</div>
                  </div>
                </div>
      ...
      <section className="py-16 border-t border-provn-border/30 overflow-hidden">
        ...
        >
          ...
          <InfiniteMarquee speed={25} pauseOnHover>
            {["Canton Network", "DAML", "Digital Asset", "Ledger API", "gRPC", "TypeScript", "React", "Next.js"].map((name) => (
              <div
                key={name}
